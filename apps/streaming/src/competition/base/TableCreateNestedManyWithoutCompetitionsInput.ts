/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { TableWhereUniqueInput } from "../../table/base/TableWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class TableCreateNestedManyWithoutCompetitionsInput {
  @Field(() => [TableWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [TableWhereUniqueInput],
  })
  connect?: Array<TableWhereUniqueInput>;
}

export { TableCreateNestedManyWithoutCompetitionsInput as TableCreateNestedManyWithoutCompetitionsInput };
